...
public interface IProductoMapper {
    @Select("select p.id_product, p.nombre_product, p.stock_compro_product, p.stock_real_product, p.imagen, p.precio_uni, p.is_precio_desc_product, p.precio_descu_product, m.vista_item as marca,\n" +
            ...
            "order by p.id_product LIMIT #{cantidad} OFFSET #{pagina};")default
    List<ProductoFilter> busquedaFiltrada (@Param("tallas") String tallas) {
        System.out.println(busquedaFiltrada(null, null, null, "`L`,`S`", null, 1, 500, 10, 0));
        return busquedaFiltrada(null, null, null, "`L`,`S`", null, 1, 500, 10, 0);
    }
